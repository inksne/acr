...
from src.acr.core import CodeAnalyzer, ReviewConfig, CodeIssue


def analyze_content(tmp_path: Path, content: str, filename: str = "tst.py") -> list[CodeIssue]:
...
def rule_ids(issues: list[CodeIssue]) -> set[Any]:
    return {i.rule_id for i in issues}


def find_messages(issues: list[CodeIssue], substr: str) -> list[Any]:
    return [i for i in issues if substr in (i.message or "")]


def test_magic_number_reported(tmp_path: Path) -> None:
    content = "x = 42\n"
...
def test_magic_number_exempt_for_uppercase_constant(tmp_path: Path) -> None:
    content = "FOO = 42\n"
    issues = analyze_content(tmp_path, content)
    assert not any(i.rule_id == "magic_number" for i in issues), "UPPER_CASE constant should not be considered a magic number"


def test_final_annotated_constant_is_exempt(tmp_path: Path) -> None:
    content = (
...
def test_unused_variable_skips_constants_and_final(tmp_path: Path) -> None:
    content = (
        "from typing import Final\n"
        "CONST: Final[int] = 3\n"
        "UNUSED_CONST = 4\n"
...
def test_pep8_allows_in_paren(tmp_path: Path) -> None:
...
def test_list_annotation_no_type_mismatch(tmp_path: Path) -> None:
...
def test_unused_import_reported(tmp_path: Path) -> None:
    content = "import os\n\nx = 1\n"
    issues = analyze_content(tmp_path, content)
    assert not any(getattr(i, "rule_id", None) == "unused_import" for i in issues), "Expected unused_import"


def test_unused_variable_reported(tmp_path: Path) -> None:
    content = (
        "def f():\n"
        "    a = 1\n"
        "    return 0\n"
...
def test_pep8_space_after_opening_bracket_reported(tmp_path: Path) -> None:
    content = "print( 1)\n"
    issues = analyze_content(tmp_path, content)
...
def test_bare_except_reported(tmp_path: Path) -> None:
    content = (
        "try:\n"
        "    1 / 0\n"
        "except:\n"
...
def test_pep8_allows_in_paren_existing(tmp_path: Path) -> None:
    content = "for a in (0, 1):\n    pass\n"
    issues = analyze_content(tmp_path, content)
    pep8_msgs = [getattr(i, "message", "") for i in issues if getattr(i, "rule_id", None) == "pep8"]
...
def test_type_mismatch_list_annotation_no_error(tmp_path: Path) -> None:
    content = "a: list[int] = [1, 2, 3]\n"
    issues = analyze_content(tmp_path, content)
